...
class L1
{
    ...
    bool search(unsigned int a)
    {
        ...
        for(j=0;j<2;j++)
        {
            if((this->s[in].b[j].tag==t)&&(this->s[in].b[j].valid==1))
            return 1;
        }
        if(j==2)
        return 0;
...
bool branch(unsigned int pc, bool result,gshare*bp1,bimodal*bp2,int*select)
{
    unsigned int k=pc;
    k=k<<20;
    k=k>>20;
...
int main()
{
    ...
    int exec=0;
     ...
     else
     {
      ...
      fp=fopen("dynamic_trace_100000","r");
     if(fp==NULL){
 ...
 while(fscanf(fp,"%x%x%d",&pc,&mem,&taken)!=EOF)
 {
      ...
      {
      br++;
      if(branch(pc,taken,&bp1,&bp2,select)==0)
      cycle=cycle+2;
      else accu++;
      }
      if(ins[pc].type==0)
